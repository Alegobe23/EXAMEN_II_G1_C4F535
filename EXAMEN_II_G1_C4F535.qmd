...
8.  Para las variables numéricas realizar gráficos de barras donde se ponga un
    color de acuerdo a si es moroso o no. Analizar los resultados.

Categoría de la edad, ordenada por décadas.

```{r}
datos.decadas <- base.datos %>% 
  mutate(DECADA = floor(EDAD/10) * 10) %>%
  group_by(DECADA) 

decadas <- unique(datos.decadas$DECADA)


for(i in decadas){
  datos.utilizar = filter(datos.decadas, DECADA == i)

  grafico = ggplot(datos.utilizar, aes(x = INDICADOR_MOROSO, fill = INDICADOR_MOROSO)) +
            geom_bar(stat = "count", width = 0.3)+
            theme_minimal()+
            labs(x = "Indicador morosidad", y = "Cantidad", 
                title=paste("Distribución de la morosidad en la década etaria:",i), fill = "Morosidad")+
            theme(plot.title = element_text(hjust = 0.5),
                  axis.title.y = element_text(margin = margin(r = 15)),
                  axis.title.x = element_text(margin = margin(t = 15)))+
            geom_text(stat = "count", aes(label = after_stat(count)), 
...
ggsave(paste0("C:/Users/alego/OneDrive/Documentos/EXAMEN_II_G1_C4F535_R/EXAMEN_II_G1_C4F535/res/Cantidad_morosidad_decada_", i, ".png"), plot = grafico, width = 8, height = 6, dpi = 300)}
```

Categoría del salario, ordenado por rangos.

```{r}
datos.salario <- base.datos %>%
  mutate(RANGO.SALARIO = case_when(
    SALARIO <= 5000000 ~ "0-5M",
    SALARIO > 5000000 & SALARIO <= 10000000 ~ "5-10M",
    SALARIO > 10000000 & SALARIO <= 15000000 ~ "10-15M",
    SALARIO > 15000000 & SALARIO <= 20000000 ~ "15-20M"
  ))

rango.salario <- unique(datos.salario$RANGO.SALARIO)

for(i in rango.salario){
  datos.utilizar = filter(datos.salario, RANGO.SALARIO == i)

  grafico = ggplot(datos.utilizar, aes(x = INDICADOR_MOROSO, fill = INDICADOR_MOROSO)) +
            geom_bar(stat = "count", width = 0.3)+
            theme_minimal()+
            labs(x = "Indicador morosidad", y = "Cantidad", 
                title=paste("Distribución de la morosidad en el rango salarial:",i), fill = "Morosidad")+
            theme(plot.title = element_text(hjust = 0.5),
                  axis.title.y = element_text(margin = margin(r = 15)),
                  axis.title.x = element_text(margin = margin(t = 15)))+
            geom_text(stat = "count", aes(label = after_stat(count)), 
...
ggsave(paste0("C:/Users/alego/OneDrive/Documentos/EXAMEN_II_G1_C4F535_R/EXAMEN_II_G1_C4F535/res/Cantidad_morosidad_rando_salarial_", i, ".png"), plot = grafico, width = 8, height = 6, dpi = 300)}
```
...
#Primero se descarta la columna del ID. Al ejecutar el árbol por primera vez, se visualizó de manera desordenada debido a los largos nombres del tipo de aseguramiento. 
base.arbol <- base.datos %>% 
  mutate(TIPO_ASEGURAMIENTO= recode(
    TIPO_ASEGURAMIENTO, 
    "PENSIONADOS EN SU PROPIO SISTEMA" = "JUBILADOS",
    "ASEGURADOS VOLUNTARIOS" = "A_VOLUNTARIOS",
    "TRABAJADOR INDEPENDIENTE" = "T_INDEPENDIENTE",
    "CONVENIOS ESPECIALES DE TRABAJADORES INDEPENDIENTES" = "CONV_INDEP",
    "CONVENIOS ESPECIALES DE ASEGURADOS VOLUNTARIOS" = "CONV_VOLUNT")) %>% 
  select(-IDENTIFICACION) 

#Se inicia la conStrucción del árbol. 
tree <- rpart(INDICADOR_MOROSO~., data = base.arbol, method = "class")
rpart.plot(tree, type = 4, extra = 104)
```

Interpretación: note que la primera variable que aparece en el árbol de decisión es el tipo de aseguramiento, un indicador de que esta variable es la más influyente en la predicción de la morosidad en los clientes.

Las personas que no pertenecen a los tipos de aseguramiento que se detallan a la izquierda del árbol de decisión, tienen un resultado predictivo negativo en el rubro de morosidad. En dado caso de que el asegurado sí pertenezca a las categorías del tipo de aseguramiento que se detallan allí, hay que proceder por la derecha, donde la siguiente interrogante consiste en si se clasifica como un trabajador independiente, en dado caso de ser negativo entran en juego dos nuevas cuestiones: la variable de la procedencia, que en caso de ser negativa, existe una división predictiva por salario de acuerdo a ciertos valores del mismo; y en dado caso de que el asegurado sea extranjero el valor predictivo es positivo. Por otro lado, si sí es trabajador independiente, se obtiene un valor predictivo positivo.
